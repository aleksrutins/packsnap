...
module NodeProvider : Provider = struct
  ...
  module PackageManager = struct
    ...
    let name = function Npm -> "npm" | Yarn -> "yarn" | Pnpm -> "pnpm"
    ...
    let detect path =
      ...
      if exists "yarn.lock" then Yarn
        ...
        log_tag
        |> warn
             "No lockfile found, assuming this project uses NPM; however, this \
              may break the build. Please run `npm install` to generate a \
              lockfile.";
        Npm)
    ...
    let get_script_cmd script pm =
      (match pm with Npm -> "run" | Yarn -> "" | Pnpm -> "") |> fun x ->
      [ name pm; x; script ] |> String.concat " "
  end
  ...
  let has_script name path =
    ...
    try
      let _ = package_json |> member "scripts" |> member name |> to_string in
      true
    with _ -> false

  let get_package_manager path = PackageManager.detect path

  let get_install_cmd path =
    PackageManager.get_install_cmd (get_package_manager path)

  let detect path = Sys.file_exists (package_json_path path)
...
let%test_module "node tests" =
  (module struct
    let%test "it detects correctly" =
    ...
    let%test "it correctly recognizes node" =
      ...
      plan.base_image = "node:18"
      && plan.build_commands = [ "npm ci" ]
      && plan.packages = []
      && plan.run_command = "node index.js"
